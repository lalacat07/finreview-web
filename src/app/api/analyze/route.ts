...
- 三个方向都要算，缺一不可：① 纵向（竖向，列内各项加总=列合计）；② 横向（行内期初+本期变动=期末，或各分项之和=行合计）；③ 逆向（反算）——当存在小计/合计且其中一项可由"合计 − 其他已知项"反推时，反算该项并与披露值核对；"根据上表年初合计数/小计项/合并报表反算"的勾稽也属此类。任一方向不符即报"竖向/横向/逆向计算有误"。
- 跨表与附注勾稽必须逐一显式比对：每张主表的每一行，与其引用的附注（如"附注六、46"）、以及与之相关的其它主表（合并↔母公司、利润表↔现金流量表↔股东权益变动表）、附注与附注之间，逐一核对同一数据；出现"与附注X一致、与附注Y不一致"这类一致/不一致并存的情形要分别列明。
- 缺失数据的处理：遇空白/缺失金额，一方面报"数据缺失（建议补充）"，另一方面在验算纵横向/逆向合计时将其视作 0 继续核验，并报出由此暴露的勾稽差异——两类问题都要列。
...
- 比例/百分比重算：持股比例、占比、毛利率、各类构成比、税率等比例数据须用对应分子分母独立重算，与披露值核对，不符报"比例计算有误"
...
1) 同一金额全文一致性（最高频，约占问题三分之一）：
   ...
   - 覆盖五种比对方向：① 主表↔附注；② 附注↔附注；③ 合并↔母公司；④ 同一张表内左右栏对应项（如"一般风险准备"列对应的"提取一般风险准备" vs "未分配利润"列对应的"提取一般风险准备"）；⑤ 主表/附注↔【审计报告·关键审计事项(KAM)】中引用的同一金额（如减值、收入确认等 KAM 段落里的数字须与对应附注一致）。
   - 跨页引用须带页码定位（如"与 PDF p9 合并资产负债表不一致"），便于复核人回溯。
...
2) 加总核验（四个方向都要算）：
   ...
   - 逆向须区分层级：分别用"小计项"和"细项"两条路径反算；若按小计反算不符但按细项反算正确，说明小计本身有误 → "根据小计项逆向计算有误，根据细项逆向计算正确，请检查"。
   - 依据其他表/小计推算：→ "根据小计项加总计算有误，请检查。" / "根据上表年初合计数计算有误，请检查。" / "根据合并股东权益变动表计算有误，请检查。" / "根据 PDF p8-9 合并资产负债表计算有误，请检查"。
   - 数据格式有误致无法运算（数字串错位、文本混入数字列、单元格格式异常）→ "由于上方/左侧数据格式有误，故无法进行竖向/横向加总计算，请检查。" / "数据格式有误，请检查"。
   - 比例/百分比重算 → "比例计算有误，请检查。"
...
16) 需管理层确认（无法仅凭报告精确重算者，归入待确认）：如适用不同税率导致汇率/利率变动对净利润/权益影响无法精确带入计算 → "由于适用不同的税率，无法带入确切的税率进行汇率变动对净利润/权益影响的计算，请检查。"

17) 币种/单位缺失：附注表格或正文金额未写明币种/计量单位（人民币、每股、千元/百万元等）→ "请考虑添加'人民币'。" / "请考虑添加币种。" / "请考虑添加'每股'。"；个别表与全篇单位披露不一致 → "其余表格均未披露单位，请考虑删除/补充，请检查"。

18) 跨页表格与版式完整性：① 跨页表格续页未重复表头/标题 → "PDF pX–Y 未披露相应表头/对应标题，请检查。" / "请考虑添加表头，请检查"；② 续表缺"(续)"标识或多余"续"标题 → "请考虑添加'续'，请检查。" / "请考虑删除带'续'的三级及以上标题，请检查"；③ 页脚缺失 → "请考虑添加页脚，请通篇检查"；④ 数据/日期被跨行截断 → "请考虑调整格式，保持日期/数据完整披露在同一行，请检查"；⑤ 多余空白行/页边距不统一 → "请考虑删除多余行空白，请统一页边距，请通篇检查"。

19) 简称/主体表述：简称首次出现须定义且全文一致，未定义即使用 → "以下未披露该简称，请考虑删除"；主体名称口径（如附注披露含"及其子公司"、应披露为"母公司"等）须前后一致 → "请考虑是否披露为'母公司'，请检查"。

【本清单的归类映射】① 第 1~4、7、16 项（金额一致性含与 KAM 一致、各类加总、数据缺失/格式影响验算、正数披露、需管理层确认）→ 输出到 "## 财务数据复核"；其中第 16 项归"待管理层确认事项"，其余归"明显问题"。② 第 5、6、8~15、17~19 项（零值列示、全半角括号、线条版式、序号编号、附注引用用语、删除冗余、主表缺"公司"、时效更新、拆分披露、年末措辞、币种单位、跨页表格版式、简称主体表述）→ 输出到 "## 语法核查"。每条问题须给出涉及页码/表名/附注号，并采用上述"示例措辞"风格，确保与人工批注一致。

【完整性要求（务必遵守）】须逐页、逐表、逐附注扫描，把发现的【每一处】问题都单独列出——同类问题在不同页/不同表重复出现时不得合并为"通篇存在"一句带过，应逐处列明页码与对象。若问题数量很多，按"明显问题→语法核查"分区后仍要尽量完整列举，不可因篇幅自行截断或抽样；确实过多时，须在该区末尾注明"等，另有 N 处同类问题"并给出代表性页码，而非静默丢弃。
...
- 括号方向（双向科目命名，高频问题，需逐项核对）：凡名称含"正向词/(反向词)"的双向科目，须按其披露金额的正负决定主词与括号词的顺序与取舍——金额为正用正向词在前、反向词加括号在后；金额为负则把表示该方向的词作为主词、另一方向放入括号。涵盖但不限于：产生/(使用)、增加/(减少)、(减少)/增加、收益/(损失)、(损失)/收益、利润/(亏损)、(亏损)/利润、净利润/(亏损)、转回/(计提)、(计提)/转回、收入/(支出)。
  · 示例：最新一年为负 → "投资活动（使用）/产生的现金流量净额"、"净利润/（亏损）"按实际方向调整；两个年度金额同向时不赘述相反项（如两年均为负的"现金及现金等价物净增加值"不写"（减少）"）。
  · 与数字方向一致性联动：括号方向须与该行数字的正负、以及"减项数据方向"全文统一；某些科目（如减值准备在特定附注表中）应以正数披露并与其他表方向一致——发现方向不一致或应为正数却列负数（反之亦然）须报出。
- 单体/母公司报表名称规范：母公司（公司层面）资产负债表/利润表/现金流量表/股东权益变动表的表名应含"公司"字样以区别于合并报表；主表与附注的名称、表头用词须全文统一规范。
