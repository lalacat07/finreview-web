...
### 偿债能力
- 评估：✅ 未发现明显异常
- 关键指标：资产负债率 29.4%；流动比率约 3.48 倍；速动比率约 2.71 倍；利息保障倍数约 114 倍（EBIT ÷ 利息费用，利息费用为示意估算）
- 简短结论：负债率低、短期偿债能力充足，账面无银行借款，利息保障倍数极高，偿债压力很小。（演示，引用公开数据）

### 现金流质量
- 评估：✅ 未发现明显异常
- 关键指标：经营活动现金流净额约 108.7 亿元（2024：49.5 亿元）；现金收益比约 0.84 倍（经营现金流 ÷ 净利润）；自由现金流约 83.7 亿元（经营现金流 − 资本开支，资本开支为示意估算）
...
/* 演示原始科目（单位：元）——主体取自泡泡玛特国际集团（9992.HK）2025 年度业绩公告
 * 的合并三大报表公开披露数据（营收、毛利、净利、资产/负债/权益、现金、存货、销售/管理
 * 开支、FY2024 资产负债表合计等均为公开数值）。利息费用、资本开支、折旧、留存收益及上期
 * 个别明细科目公开摘要未逐项列示，此处以与公开数据自洽的示意估算补足，使「指标重算」中的
 * 利息保障倍数、自由现金流、Altman Z-Score、Beneish M-Score 等可完整呈现。
...
const DEMO_FIGURES: FigureInput = {
  ...
  financeCost: 150_000_000, // 示意估算：主要为租赁负债利息，公司无银行借款
  sga: 9_850_000_000, // 经销及销售开支 80.8 亿 + 一般及行政开支 17.7 亿（公开披露）
  depreciation: 1_900_000_000, // 示意估算：固定资产折旧 + 使用权资产折旧 + 摊销
  // 合并资产负债表（2025-12-31）
  ...
  cash: 13_775_090_000,
  retainedEarnings: 16_500_000_000, // 示意估算：未分配利润 + 盈余储备
  ppeNet: 2_000_000_000, // 示意估算：物业、厂房及设备净额
  // 合并现金流量表（FY2025）
  operatingCashFlow: 10_865_000_000,
  capex: 2_500_000_000, // 示意估算：购建物业厂房设备及无形资产支付的现金
  // 每股
  weightedShares: 1_343_000_000,
  // 对比期（FY2024）
  prevRevenue: 13_037_750_000,
  prevGrossProfit: 8_709_000_000, // 13,037.75 × 66.8%（公开毛利率）
  prevNetProfit: 3_308_000_000,
  prevNetProfitToParent: 3_125_473_000,
  prevOperatingCashFlow: 4_954_220_000,
  prevTotalAssets: 14_870_672_000, // FY2024 资产总计（公开披露）
  prevTotalLiabilities: 3_986_033_000, // FY2024 负债合计（公开披露）
  prevCurrentAssets: 11_500_000_000, // 示意估算
  prevAccountsReceivable: 480_000_000, // 示意估算
  prevPpeNet: 1_100_000_000, // 示意估算
  prevDepreciation: 950_000_000, // 示意估算
  prevSga: 4_600_000_000, // FY2024 销售 36.5 亿 + 管理 9.5 亿（公开披露）
}
const DEMO_PAGES: Record<string, string> = {
  ...
  interestCover: '2025 年度业绩公告 · 损益表 / 财务费用附注（利息为示意估算）',
  fcf: '2025 年度业绩公告 · 合并现金流量表（资本开支为示意估算）',
